...
on_plugin_menu[PLUGIN_NAME] = ((menu) => {
    ...
    createToggleMenu(menu, '选中多个文件时启用', 'multifile.enable', sub => {
        ...
        const spliter = sub.append_menu({
            name: '多个文件路径分隔符: ' + spliters[read_config_key('multifile.copyall_split')],
            action() {
...
shell.menu_controller.add_menu_listener(ctx => {
    if (ctx.context.folder_view) {
        ...
        let target = items.findIndex(v => v.data().name === '复制文件地址' || v.data().name === 'Copy as path')

        let menu;
        if (target === -1) {
            target = items.findIndex(v => v.data().name === '属性') + 1
